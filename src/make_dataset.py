...
def prepare_data(input_folder: str, output_folder: str) -> None:
    ...
    titanic_train = pd.DataFrame(X_train, columns=preprocessing.get_feature_names_out(), index=train_data.index)
    titanic_dev = pd.DataFrame(X_dev, columns=preprocessing.get_feature_names_out(), index=dev_data.index)
    titanic_test = pd.DataFrame(X_test, columns=preprocessing.get_feature_names_out(), index=test_data.index)
